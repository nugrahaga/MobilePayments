...
@SuppressLint("WorldWriteableFiles") public class FileOperation {
	private Context ct;
	...
	public boolean copyFile(int resourceId, String folder) {
		try {
			if (resourceId != 0) {
				...
				InputStream src = ct.getResources().openRawResource(resourceId);
				String filename = ct.getResources().getResourceEntryName(
						resourceId);// get file name;
				...
				if (!fl.exists()) {
					FileOutputStream stream = ct.openFileOutput(filename,
	...
	public boolean downLoadFile(String url, String localfilename) {
		...
		try {
			...
			FileOutputStream fos = ct.openFileOutput(localfilename,
					Context.MODE_WORLD_WRITEABLE);
			...
			try {
				...
				Log.e("tag", "error: " + ex.getMessage() +" "+downLoadFileSize, ex);
			}
		} catch (Exception e) {
			e.printStackTrace();
			return false;
